...
{
    public $modelClass = LostLight::class;
...
//    }
    ...
    public function actionSend()
    {
        ...
        foreach ($items as $item) {
            $model = LostLight::find()->where(['uuid' => $item['uuid']])->one();
            if ($model == null) {
                $model = new LostLight();
//                $model->_id = $item['_id'];
            ...
            $model->load($item, '');
            $model->objectAddress = $model->getObjectFullAddress();
            $model->nodeAddress = $model->getNodeFullAddress();

            if (!$model->save()) {
                throw new HttpException(401, 'lost_light not saved.');
            }
        }
